...
class Generative_AI_Model:
    ...
    def __repr__(self):
        return "This is an object of Generate response to user query using Generative AI"
    
    # Store the generated model response in MongoDB with user query
    def stored_session_history_in_MongoDB(self, model_type, user_query, model_response) -> None:
        data = {
    ...
    def generate_response_according_selected_model_type(self, model_type ,user_query):
        try:
            if model_type == "OpenAI":
                ...
                self.stored_session_history_in_MongoDB(model_type, user_query, model_response)
                return JSONResponse(content={"message":model_response}, status_code=200)
    ...
    def get_session_history_from_MongoDB(self):
        try:
            session_history = list(self.mongodb_session_history_collection.find({}, projection={"_id": 0, "user_query": 1, "model_response": 1}).sort("_id", -1).limit(10))
